...
function get_changes {
        declare -a args
        args=(-type f -mmin -1 -not -path '*/.venv/*' -not -path '*/.git/*' -not -path '*/*data/*' -not -path "*.png" -not -path '*.lock' -not -path '*.gitignore' -not -path "*/lib/*" -not -path "*/target/*", -not -path "*/*.qgz" -not -path "*/*.pdf" -not -path "*/*.aux" -not -path "*/*.fdb_latexmk" -not -path "*/*.synctex.gz" -not -path "*/*.log" -not -path "*/*.fls" -not -path "*/*.log")
        sum=0
...
function create_row {
        ...
        printf '%s,%s,%s,%s,%s,%s\n' "$1" "$2" "$3" "$fatigue" "$mood" "$bpm" >> "../logs/log.csv"
}
...
done < <(playerctl --follow status -p spotify)
