...
function get_changes {
        declare -a args
        diff_in_secs=$(( "$1" / 60 ))
        args=(-type f -mmin -"$diff_in_secs" -not -path '*/.venv/*' -not -path '*/.git/*' -not -path '*/*data/*' -not -path "*.png" -not -path '*.lock' -not -path '*.gitignore' -not -path "*/lib/*" -not -path "*/target/*", -not -path "*/*.qgz" -not -path "*/*.pdf" -not -path "*/*.aux" -not -path "*/*.fdb_latexmk" -not -path "*/*.synctex.gz" -not -path "*/*.log" -not -path "*/*.fls" -not -path "*/*.log" -not -path "*/*.rkt~")
        sum=0
...
function sync_changes {
        latest=$(tail -n 1 "../logs/log.csv")
        IFS=',' read -ra record <<< "$latest"
        current=$(date +"%s")
        diff=$(($current - ${record[1]}))
        get_changes $diff
}
